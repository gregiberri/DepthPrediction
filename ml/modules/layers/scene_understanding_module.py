...
from ml.modules.layers.basic_layers import conv_bn_relu
...
class FullImageEncoder(nn.Module):
    def __init__(self, h, w, kernel_size, dropout_prob=0.5):
        ...
        self.global_fc = nn.Linear(512 * self.h * self.w, 512)  # kitti 4x5
        self.relu = nn.ReLU(inplace=True)
        # TODO: i dont think we need to downsample to 1x1 shape
    ...
    def forward(self, x):
        ...
        x2 = self.dropout(x1)
        x3 = x2.view(-1, 512 * self.h * self.w)  # kitti 4x5
...
class SceneUnderstandingModule(nn.Module):
    def __init__(self, ord_num, size, kernel_size, pyramid=[6, 12, 18], dropout_prob=0.5, batch_norm=False):
        ...
        self.aspp2 = nn.Sequential(
            conv_bn_relu(batch_norm, 512, 512, kernel_size=3, padding=pyramid[0], dilation=pyramid[0]),
            conv_bn_relu(batch_norm, 512, 512, kernel_size=1, padding=0)
        )
        self.aspp3 = nn.Sequential(
            conv_bn_relu(batch_norm, 512, 512, kernel_size=3, padding=pyramid[1], dilation=pyramid[1]),
            conv_bn_relu(batch_norm, 512, 512, kernel_size=1, padding=0)
        )
        self.aspp4 = nn.Sequential(
            conv_bn_relu(batch_norm, 512, 512, kernel_size=3, padding=pyramid[2], dilation=pyramid[2]),
            conv_bn_relu(batch_norm, 512, 512, kernel_size=1, padding=0)
        )
        self.concat_process = nn.Sequential(
            nn.Dropout2d(p=dropout_prob),
            conv_bn_relu(batch_norm, 512 * 4, 2048, kernel_size=1, padding=0),
            nn.Dropout2d(p=dropout_prob),
