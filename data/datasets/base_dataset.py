...
class BaseDataset(data.Dataset):
    def __init__(self, config, is_train=True, image_loader=None, depth_loader=None):
        ...
        self.aug = iaa.Sequential([iaa.Fliplr(aug_config.random_flip),
                                   iaa.AdditiveGaussianNoise(0, scale=aug_config.random_noise),
                                   iaa.GaussianBlur(sigma=(0, aug_config.gauss_blur)),
                                   iaa.AddToHueAndSaturation(value=(-aug_config.random_hue_saturation,
                                                                    aug_config.random_hue_saturation)),
                                   iaa.TranslateX(px=(-aug_config.random_shift,
                                                      aug_config.random_shift)),
                                   iaa.TranslateY(px=(-aug_config.random_shift,
                                                      aug_config.random_shift))])
